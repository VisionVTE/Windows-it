# PowerShell Driver Installation Script for Apple A18 Pro Storage Driver
# Requires Administrator privileges

$ErrorActionPreference = "Stop"

function Write-Status ($message, $color = "Green") {
    Write-Host "[Apple A18 Pro Storage] $message" -ForegroundColor $color
}
...
Write-Status "==================================================" "Cyan"
Write-Status "Apple A18 Pro Storage Driver Installation" "Cyan"
Write-Status "==================================================" "Cyan"
...
foreach ($path in $searchPaths) {
    if (Test-Path (Join-Path $path "AppleA18ProStorage.inf")) {
        $infSource = Join-Path $path "AppleA18ProStorage.inf"
    }
    if (Test-Path (Join-Path $path "AppleA18ProStorage.sys")) {
        $sysSource = Join-Path $path "AppleA18ProStorage.sys"
    }
}

if (-not $infSource -or -not $sysSource) {
    Write-Status "Could not find driver source files." "Red"
    Write-Status "Required: 'AppleA18ProStorage.inf' and 'AppleA18ProStorage.sys'" "Red"
    Exit 1
}
...
Copy-Item $infSource -Destination (Join-Path $stageDir "AppleA18ProStorage.inf") -Force
Copy-Item $sysSource -Destination (Join-Path $stageDir "AppleA18ProStorage.sys") -Force
...
if ($null -ne $inf2catPath) {
    ...
    try {
        ...
        Write-Status "Catalog file AppleA18ProStorage.cat generated successfully." "Green"
    } catch {
    ...
    $catSource = $null
    foreach ($path in $searchPaths) {
        if (Test-Path (Join-Path $path "AppleA18ProStorage.cat")) {
            $catSource = Join-Path $path "AppleA18ProStorage.cat"
            break
        }
    }
    if ($null -ne $catSource) {
        Copy-Item $catSource -Destination (Join-Path $stageDir "AppleA18ProStorage.cat") -Force
...
try {
    $sysPath = Join-Path $stageDir "AppleA18ProStorage.sys"
    $catPath = Join-Path $stageDir "AppleA18ProStorage.cat"
...
Write-Status "Registering and installing storage driver..." "Cyan"
$infPath = Join-Path $stageDir "AppleA18ProStorage.inf"
try {
...
Write-Status "==================================================" "Cyan"
Write-Status "Storage installation complete!" "Green"
