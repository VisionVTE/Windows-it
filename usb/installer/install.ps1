# PowerShell Driver Installation Script for Apple A18 Pro USB Driver
# Requires Administrator privileges

$ErrorActionPreference = "Stop"

function Write-Status ($message, $color = "Green") {
    Write-Host "[Apple A18 Pro USB] $message" -ForegroundColor $color
}
...
Write-Status "==================================================" "Cyan"
Write-Status "Apple A18 Pro USB Driver Installation" "Cyan"
Write-Status "==================================================" "Cyan"
...
foreach ($path in $searchPaths) {
    if (Test-Path (Join-Path $path "AppleA18ProUsb.inf")) {
        $infSource = Join-Path $path "AppleA18ProUsb.inf"
    }
    if (Test-Path (Join-Path $path "AppleA18ProUsb.sys")) {
        $sysSource = Join-Path $path "AppleA18ProUsb.sys"
    }
}

if (-not $infSource -or -not $sysSource) {
    Write-Status "Could not find driver source files." "Red"
    Write-Status "Required: 'AppleA18ProUsb.inf' and 'AppleA18ProUsb.sys'" "Red"
    Exit 1
}
...
Copy-Item $infSource -Destination (Join-Path $stageDir "AppleA18ProUsb.inf") -Force
Copy-Item $sysSource -Destination (Join-Path $stageDir "AppleA18ProUsb.sys") -Force
...
if ($null -ne $inf2catPath) {
    ...
    try {
        ...
        Write-Status "Catalog file AppleA18ProUsb.cat generated successfully." "Green"
    } catch {
    ...
    $catSource = $null
    foreach ($path in $searchPaths) {
        if (Test-Path (Join-Path $path "AppleA18ProUsb.cat")) {
            $catSource = Join-Path $path "AppleA18ProUsb.cat"
            break
        }
    }
    if ($null -ne $catSource) {
        Copy-Item $catSource -Destination (Join-Path $stageDir "AppleA18ProUsb.cat") -Force
...
try {
    $sysPath = Join-Path $stageDir "AppleA18ProUsb.sys"
    $catPath = Join-Path $stageDir "AppleA18ProUsb.cat"
...
Write-Status "Registering and installing USB driver..." "Cyan"
$infPath = Join-Path $stageDir "AppleA18ProUsb.inf"
try {
...
Write-Status "==================================================" "Cyan"
Write-Status "USB installation complete!" "Green"
