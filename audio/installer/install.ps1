# PowerShell Driver Installation Script for Apple A18 Pro Audio Driver
# Requires Administrator privileges

$ErrorActionPreference = "Stop"

function Write-Status ($message, $color = "Green") {
    Write-Host "[Apple A18 Pro Audio] $message" -ForegroundColor $color
}
...
Write-Status "==================================================" "Cyan"
Write-Status "Apple A18 Pro Audio Driver Installation" "Cyan"
Write-Status "==================================================" "Cyan"
...
foreach ($path in $searchPaths) {
    if (Test-Path (Join-Path $path "AppleA18ProAudio.inf")) {
        $infSource = Join-Path $path "AppleA18ProAudio.inf"
    }
    if (Test-Path (Join-Path $path "AppleA18ProAudio.sys")) {
        $sysSource = Join-Path $path "AppleA18ProAudio.sys"
    }
}

if (-not $infSource -or -not $sysSource) {
    Write-Status "Could not find driver source files." "Red"
    Write-Status "Required: 'AppleA18ProAudio.inf' and 'AppleA18ProAudio.sys'" "Red"
    Exit 1
}
...
Copy-Item $infSource -Destination (Join-Path $stageDir "AppleA18ProAudio.inf") -Force
Copy-Item $sysSource -Destination (Join-Path $stageDir "AppleA18ProAudio.sys") -Force
...
if ($null -ne $inf2catPath) {
    ...
    try {
        ...
        Write-Status "Catalog file AppleA18ProAudio.cat generated successfully." "Green"
    } catch {
    ...
    $catSource = $null
    foreach ($path in $searchPaths) {
        if (Test-Path (Join-Path $path "AppleA18ProAudio.cat")) {
            $catSource = Join-Path $path "AppleA18ProAudio.cat"
            break
        }
    }
    if ($null -ne $catSource) {
        Copy-Item $catSource -Destination (Join-Path $stageDir "AppleA18ProAudio.cat") -Force
...
try {
    $sysPath = Join-Path $stageDir "AppleA18ProAudio.sys"
    $catPath = Join-Path $stageDir "AppleA18ProAudio.cat"
...
Write-Status "Registering and installing audio driver..." "Cyan"
$infPath = Join-Path $stageDir "AppleA18ProAudio.inf"
try {
...
Write-Status "==================================================" "Cyan"
Write-Status "Audio installation complete!" "Green"
